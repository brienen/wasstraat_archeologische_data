...
COLLECT_PIPELINES = "collect_pipelines"
EXTRACT_PIPELINES = "extract_pipelines"
...
wasstraat_model["Plaatsing"][COLLECT_PIPELINES] = wasstraat_model['Magazijnlocatie']['collect_pipelines']
wasstraat_model["Doos"][COLLECT_PIPELINES] = [wasstraat_model['Magazijnlocatie']['collect_pipelines'][0]]



def getCollectPipeline(soort):
    if soort in wasstraat_model.keys():
        return wasstraat_model[soort][COLLECT_PIPELINES][0]
    else:
        raise Exception(f'Fout bij het opvragen van metadata. {soort} is een onbekend metadatasoort.')

def getCollectPipelines(soort):
    if soort in wasstraat_model.keys():
        return wasstraat_model[soort][COLLECT_PIPELINES]
    else:
        raise Exception(f'Fout bij het opvragen van metadata. {soort} is een onbekend metadatasoort.')
...
def getVeldnamen(soort):    
    keyset = set([])
    lst_pipelines = []
    for p in  wasstraat_model[soort][COLLECT_PIPELINES]:
        lst_pipelines += p
    for p in  wasstraat_model[soort][EXTRACT_PIPELINES]:
        lst_pipelines += p
...
def getKeys(fase):
    if not fase in [COLLECT_PIPELINES, EXTRACT_PIPELINES]:
